...
from app.schemas import Report, ReportCreate, ReportUpdate, ReportQueryFilters
...
async def list_reports(
    ...
    """
    logger.info(f"User {current_user.email} fetching reports list with filters: {filters.model_dump(exclude_none=True)}")
...
async def read_report(
    ...
    Requires authentication.
    """
    logger.info(f"User {current_user.email} fetching report metadata for ID: {report_id}")
...
async def trigger_report_generation(
    ...
    db: AsyncSession = Depends(get_db), # May not be needed directly if task handles DB interaction
    current_user: User = Depends(get_current_active_superuser), # Restrict who can trigger generation
    ...
    generation_params = {"time_range": "last_7_days", "triggered_by": current_user.email}
