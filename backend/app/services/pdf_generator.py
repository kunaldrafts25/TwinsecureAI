...
from app.schemas import ReportCreate, AlertQueryFilters, Report # Import Report schema
...
if PDF_LIBRARY == "reportlab":
    try:
        ...
        logger.error("ReportLab not installed. PDF generation will fail. Install with 'pip install reportlab'")
        PDF_LIBRARY = None
    ...
    try:
        ...
        logger.error("WeasyPrint not installed or system dependencies missing. PDF generation will fail. Install with 'pip install weasyprint' and ensure system libs are present.")
        PDF_LIBRARY = None
     ...
     try:
        ...
        logger.error("pdfkit not installed or wkhtmltopdf not found. PDF generation will fail. Install with 'pip install pdfkit' and ensure wkhtmltopdf is installed.")
        PDF_LIBRARY = None
else:
    logger.error(f"Invalid PDF_LIBRARY selected: {PDF_LIBRARY}. PDF generation disabled.")
    PDF_LIBRARY = None
...
def _generate_pdf_reportlab(filename: str, title: str, start_time: datetime, end_time: datetime, alerts_data: list, aggregates: dict):
    ...
    recommendations = aggregates.get("recommendations", "No specific recommendations generated.")
...
async def generate_report_pdf(params: dict):
    ...
    """
    if not PDF_LIBRARY:
        logger.error("No valid PDF library configured or available. Cannot generate report.")
        return
    ...
    try:
        async with AsyncSessionLocal() as db:
            ...
            for alert in alerts_data:
                ...
                if ip: top_attackers[ip] = top_attackers.get(ip, 0) + 1
            ...
            aggregates["top_attackers"] = sorted(top_attackers.items(), key=lambda item: item[1], reverse=True)
            aggregates["alerts_by_country"] = sorted(alerts_by_country.items(), key=lambda item: item[1], reverse=True)
            # Add more aggregations...

            # --- Generate Recommendations (Example) ---
            recs = []
            if aggregates["top_attackers"]:
                 recs.append(f"- Review activity from top attacking IPs: {', '.join([ip for ip, count in aggregates['top_attackers'][:3]])}.")
                 # Consider threshold for blocking recommendation
                 if aggregates["top_attackers"][0][1] > 50: # If top attacker has > 50 alerts
                      recs.append(f"- Consider blocking IP {aggregates['top_attackers'][0][0]} due to high alert volume ({aggregates['top_attackers'][0][1]} alerts).")
            if aggregates["alerts_by_country"]:
                 recs.append(f"- Monitor traffic from top source countries: {', '.join([c for c, count in aggregates['alerts_by_country'][:3]])}.")
            aggregates["recommendations"] = "\n".join(recs) if recs else "No specific automated recommendations."

    except Exception as e:
        logger.error(f"Error fetching/aggregating data for PDF report: {e}", exc_info=True)
        return
    ...
    try:
        if PDF_LIBRARY == "reportlab":
            await loop.run_in_executor(None, _generate_pdf_reportlab, local_pdf_path, report_title, report_start_time, report_end_time, alerts_data, aggregates)
            ...
            if PDF_LIBRARY == "weasyprint":
                 await loop.run_in_executor(None, _generate_pdf_weasyprint, local_pdf_path, html_content)
                 pdf_generation_successful = True
            elif PDF_LIBRARY == "pdfkit":
                 await loop.run_in_executor(None, _generate_pdf_pdfkit, local_pdf_path, html_content)
                 pdf_generation_successful = True

        if pdf_generation_successful:
            logger.info(f"PDF report generated successfully using {PDF_LIBRARY} at: {local_pdf_path}")
        else:
             logger.error("PDF generation skipped due to library configuration issue.")
             return # Exit if PDF wasn't generated

    except Exception as e:
        logger.error(f"Error generating PDF content with {PDF_LIBRARY}: {e}", exc_info=True)
        if os.path.exists(local_pdf_path): os.remove(local_pdf_path) # Clean up partial file
    ...
    try:
        ...
        async with AsyncSessionLocal() as db:
            ...
            logger.info(f"Report record created in database with ID: {created_report.id}")
    ...
    if created_report_model:
        try:
            ...
            download_url = f"Access via API or path: {file_location_uri}" # Simple placeholder
            report_notification_data["download_url"] = download_url

            await alert_client.send_report_notification(report_data=report_notification_data)
        except Exception as e:
            logger.error(f"Failed to send report notification for report ID {created_report_model.id}: {e}")

        finally:
        # Clean up local temporary file if it wasn't the final storage location
            if file_location_uri and not file_location_uri.startswith("file://") and os.path.exists(local_pdf_path):
             try:
                 os.remove(local_pdf_path)
                 logger.debug(f"Removed temporary local PDF file: {local_pdf_path}")
             except OSError as e:
                 logger.error(f"Error removing temporary PDF file {local_pdf_path}: {e}")
