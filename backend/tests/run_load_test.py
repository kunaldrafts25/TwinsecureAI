...
def parse_args():
    ...
    parser.add_argument(
        ...
        help=f"Host to load test (default: {DEFAULT_HOST})",
    )
    parser.add_argument(
        "--users",
        type=int,
    ...
    parser.add_argument(
        ...
        help=f"Rate at which users are spawned (default: {DEFAULT_SPAWN_RATE})",
    )
    parser.add_argument(
        "--run-time",
        default=DEFAULT_RUN_TIME,
        help=f"Duration of the test (default: {DEFAULT_RUN_TIME})",
    )
    parser.add_argument(
        "--report-dir",
    ...
    parser.add_argument(
        "--exclude-tags", help="Comma-separated list of tags to exclude"
    )
...
def check_locust_installed():
    ...
    try:
        ...
        logger.error(
            "Locust is not installed. Please install it with 'pip install locust'."
        )
        return False
