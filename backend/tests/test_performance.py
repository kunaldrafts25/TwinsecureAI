...
# Import fixtures
from tests.conftest import client, user_auth_headers, superuser_auth_headers, performance_timer
...
def test_health_endpoint_performance(client: TestClient, performance_timer):
    ...
    assert max_time < MAX_RESPONSE_TIME, f"Maximum response time ({max_time:.4f}s) exceeds threshold ({MAX_RESPONSE_TIME}s)"
    assert avg_time < MAX_AVERAGE_RESPONSE_TIME, f"Average response time ({avg_time:.4f}s) exceeds threshold ({MAX_AVERAGE_RESPONSE_TIME}s)"
...
def test_alerts_endpoint_performance(client: TestClient, user_auth_headers: Dict[str, str], performance_timer):
...
def test_create_alert_performance(client: TestClient, user_auth_headers: Dict[str, str], performance_timer):
