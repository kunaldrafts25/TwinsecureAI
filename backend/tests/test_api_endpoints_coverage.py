...
def test_api_v1_alerts_update(client, user_auth_headers):
    ...
    update_data = {"status": "ACKNOWLEDGED", "severity": "MEDIUM"}
    response = client.patch(
        f"/api/v1/alerts/{alert_id}", json=update_data, headers=user_auth_headers
    )
    assert response.status_code == 200
    data = response.json()
    assert data["id"] == alert_id
...
def test_api_v1_alerts_delete(client, superuser_auth_headers):
    ...
    create_response = client.post(
        "/api/v1/alerts/", json=alert_data, headers=superuser_auth_headers
    )
    assert create_response.status_code == 200
    alert_id = create_response.json()["id"]
    ...
    response = client.delete(
        f"/api/v1/alerts/{alert_id}", headers=superuser_auth_headers
    )
    assert response.status_code == 200
